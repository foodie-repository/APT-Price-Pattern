...
### 2.1 백테스트용 기본 상승확률 점수

- 시장 국면 점수: `최근 6개월 가격 과열 역순`, `최근 12개월 가격 과열 역순`, `최근 6개월 거래회복률`, `최근 12개월 유동성`
- 임차 지지 점수: `전세가율`, `전세 12개월 변화율`, `월세 비중`
- 공급/미분양 점수: `최근 12개월 공급부담`, `총 미분양/미분양 압박`, `준공후 미분양 비중`
- 가중치:
  - 시장 국면 `40`
  - 임차 지지 `35`
  - 공급/미분양 `25`
- 현재 후보 점수는 여기에 아래 보강 점수를 추가했다.

### 2.2 현재 시점 보강 점수

- 현재 시점 점수는 기본 정량모델 위에 `조건부 저평가`, `호가`, `향후 입주예정 압력`을 덧붙인 구조다.
- 정책/규제 축은 현재 `정책메모`로 따로 붙였고, 안정적인 장기 처리군-대조군 패널이 없어서 이번 단계에서는 별도 숫자 점수로 넣지 않았다.
- 투자 적합성 점수:
  - `최근 12개월 거래량`, `시장 규모(세대수)`, `가격 수준`을 함께 본다.
  - 방향성은 좋아 보여도 `유동성 부족` 또는 `시장 규모 부족`이면 투자 후보에서 제외한다.
- 호가 보조지표:
  - `가격기준 급매`, `호가-실거래 괴리`, `매물 수`는 후보 제외가 아니라 `급매 여부 확인`, `현재 체감 분위기`, `실제 매수 직전 점검` 용도로만 사용한다.
- 저평가 가능 점수:
  - 생활권 동급 괴리, `저평가가능` 비중, 전세가율, 최근 12개월 가격변화 역순, 호가 괴리
- 촉매 점수:
  - 거래 회복, 낮은 미래 입주압력, 낮은 준공후 미분양 비중, 전세 강세, 가격기준 급매 비중
- 수익률 후보 점수:
  - 전세가율, 절대가격 부담 역순, 미래 공급압력 역순, 임차 지지
- 과열 가능 점수:
  - 최근 가격 급등, 호가 괴리, 과대반영 비중, 미래 입주압력, 낮은 전세가율

### 2.3 공급 입력 반영 원칙

- 과거 공급부담: `사용승인년월 + 세대수` 기반 최근 12개월 공급부담
- 현재 압박: `총 미분양`, `준공후 미분양`, `준공후 미분양 비중`
- 미래 압력: `입주물량` 기준 향후 `6개월`, `18개월` 입주예정 부담
