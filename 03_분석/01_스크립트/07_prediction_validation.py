...
### 2.6 현재 버전에서 직접 점수화하지 않은 보조축

- `HUG` 분양지표, `KOSIS` 공식 심리, `KB` 민간 심리, `BOK` 금리·대출 지표는 이번 버전에서 `시군구 직접 점수`에 기계적으로 더하지 않았다.
- 이유는 아래와 같다.
  - 상당수가 전국 또는 시도 단위라 시군구 횡단면 점수와 공간 해상도가 맞지 않는다.
  - 분양시장, 심리, 금융조건은 국면 전환 설명력은 크지만 직접 점수화 시 시점 누수와 과적합 위험이 있다.
- 그래서 현재는 아래 용도로 우선 사용한다.
  - `현재 장세 배경` 설명
  - `상승/보합/하락` 시나리오의 촉발 조건과 무효화 조건 보강
  - `정책 완화가 왜 바로 거래로 번역되지 않는지` 같은 상쇄 요인 해석
- 향후에는 `미반영`, `국면 보정`, `직접 점수화` 3개 버전을 비교 검증한 뒤에만 제한적으로 점수화 여부를 결정한다.
